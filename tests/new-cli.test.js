...
await asyncTest('CLI integrations command should work', async () => {
  ...
  assertTrue(result.stdout.length > 0 || result.stderr.length > 0, 'Should provide integrations output');
});
...
await asyncTest('CLI init should handle test scenarios', async () => {
  ...
  try {
    ...
    if (result.success) {
      assertTrue(result.stdout.includes('✅') || result.stdout.includes('completed'), 
        'Successful init should show completion');
    } else {
      // If it fails, should be a controlled failure with helpful message
      assertTrue(result.stderr.length > 0 || result.stdout.length > 0, 
...
await asyncTest('CLI deploy command should be available', async () => {
  ...
  assertTrue(result.success || result.stdout.includes('deploy'), 'Deploy command should be available');
});
...
await asyncTest('CLI update command should be available', async () => {
  ...
  assertTrue(result.success || result.stdout.includes('update'), 'Update command should be available');
});
...
await asyncTest('CLI claude-code command should be available', async () => {
  ...
  assertTrue(result.success || result.stdout.includes('claude-code'), 'Claude Code integration should be available');
});
