...
await asyncTest('ProjectScanner should validate file paths', async () => {
  ...
  for (const dangerousPath of dangerousPaths) {
    ...
    assertTrue(result.error || result.files.length === 0, 
...
await asyncTest('File analyzers should handle malformed content safely', async () => {
  ...
  for (const input of malformedInputs) {
    try {
      ...
      assertTrue(typeof jsResult === 'object', 'JavaScript analyzer should return object for malformed input');
      
      const tsResult = await analyzeTypeScript(input, 'test.ts');  
      assertTrue(typeof tsResult === 'object', 'TypeScript analyzer should return object for malformed input');
    } catch (error) {
...
await asyncTest('CLI should validate command arguments', async () => {
  ...
  assertTrue(cliSource.includes('version') || cliSource.includes('Version'), 'CLI should have version info');
...
await asyncTest('PatternDetector should limit resource usage', async () => {
  ...
  assertTrue(Array.isArray(patterns.architecturalPatterns), 'Should return architectural patterns array');
