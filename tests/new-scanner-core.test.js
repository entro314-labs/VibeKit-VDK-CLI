...
await asyncTest('DependencyAnalyzer.analyzeDependencies should build dependency graph', async () => {
...
await asyncTest('Full scanner pipeline should work end-to-end', async () => {
  ...
  assertTrue(Object.keys(patterns.namingConventions).length > 0, 'Pattern detection found conventions');
  assertTrue(dependencies.moduleCount >= 0, 'Dependency analysis completed');
  
  console.log(`  📊 Scanned ${projectStructure.files.length} files in ${projectStructure.directories.length} directories`);
  console.log(`  📊 Found ${patterns.architecturalPatterns.length} architectural patterns`);
  console.log(`  📊 Analyzed ${dependencies.moduleCount} modules with ${dependencies.edgeCount} dependencies`);
});
